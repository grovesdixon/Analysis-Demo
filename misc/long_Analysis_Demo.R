...
ll = load('data/full_rld.Rdata')
rld_df[1:8,1:5]
dim(rld_df)


#load sample trait data
coldata = read_csv('data/full_coldata.csv')
coldata
dim(coldata)

#format project names for plotting
lab_df = coldata %>% 
  group_by(project) %>% 
  summarize(N=n()) %>% 
  mutate(formN = paste('(N=', N, ')', sep=''),
         lab = paste(project, formN))
legend_labs = lab_df$lab
names(legend_labs) = lab_df$project
coldata$legend_lab = legend_labs[coldata$project]
coldata$my_stage = factor(coldata$my_stage,
                          levels=c('gamete',
                                   'embryo',
                                   'larva',
                                   'recruit',
...
#build project PCA
pca_df = build_rld_pca(df = rld_df,
                         coldata = coldata,
                         ntop = 1000,
                         pcs=2)
...
#plot for project
proj_plt = plot_rld_pca(pca_df,
                        group_col = 'legend_lab',
                        x_invert=-1,
                        legend_position = 'right')
proj_plt


#plot for developmental stage
stage_plt = plot_rld_pca(pca_df,
                         group_col = 'my_stage',
                         x_invert=-1,
                         legend_position = 'right')


#get stage PCA colors
stage_build <- ggplot_build(stage_plt)
pca_colors = stage_build$data[[1]] %>% 
  filter(!duplicated(group)) %>% 
  arrange(desc(group))
select_stages = c('larva', 'recruit', 'adult')

#plot density for three main stages
stage_density = pca_df %>% 
  filter(my_stage %in% select_stages) %>% 
  mutate(`developmental stage` = factor(my_stage,
                                        levels=select_stages),
         PC1 = PC1*-1) %>% 
  ggplot(aes(x=PC1, fill=`developmental stage`)) +
  geom_density() +
  scale_fill_manual(values=pca_colors$colour[c(3,2,1)]) +
  theme(legend.position = 'none')

#assemble panels and plot together
bottom = plot_grid(stage_plt, stage_density, rel_widths = c(1,.75))
plot_grid(proj_plt, bottom,
          nrow=2)
...
#look at how often they are wrong
head(rfres)
...
# APPLY CLUSTERS TO PCA ---------------------------------------------------

#replot PCA
spca_df %>% 
  mutate(cluster = 'control',
         cluster = if_else(stress == 'stressed' & my_title %in% clus_a,
                           'cluster A stress',
                           cluster),
         cluster = if_else(stress == 'stressed' & my_title %in% clus_b,
                           'cluster B stress',
                           cluster),
         cluster = factor(cluster, levels=c('control',
                                            'cluster A stress',
                                            'cluster B stress'))) %>% 
  ggplot(aes(x=PC1,
             fill=cluster)) +
  geom_density(alpha=0.75) +
  scale_fill_manual(values = c(control_col,
                               cluster_a_col,
                               cluster_b_col))
